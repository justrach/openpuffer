...
./zig-out/bin/zvec bench-synthetic --n 20000 --queries 200 --dim 1536 --k 10 --ef 128
...
./zig-out/bin/zvec bench-live --namespace zvec-bench-5 \
...
| dataset | zvec p50 | zvec recall@10 | turbopuffer p50 | tpuf recall@10 | zvec speedup |
|---|---|---|---|---|---|
| 1000 × 1536, ef=256 | **0.72 ms** | 0.960 | 213 ms | 1.000 | ~295x latency |
| 1000 × 3072, ef=256 | **1.01 ms** | 0.965 | 216 ms | 1.000 | ~215x latency |

Caveat: zvec is an in-process library (no network hop), turbopuffer a cloud
service measured over WAN — the multiple reflects access model as much as
engine. The apples-to-apples local comparison:

### Local: zvec ANN (int8 traversal + f32 rerank) vs exact brute-force scan

| dataset | zvec p50 | exact scan p50 | speedup | recall@10 |
|---|---|---|---|---|
