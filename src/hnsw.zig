...
pub fn Hnsw(comptime D: type) type {
    ...
    return struct {
        ...
        /// Drop the last 128 i8 dims on upper layers (2 AVX-512 chunks).
        /// Layer 0 does almost all evals — drop 192 (3 chunks) there.
        fn searchPrefix(self: *const Self, layer: u32) usize {
            if (self.dim < 512) return self.dim;
            const drop: usize = if (layer == 0) 192 else 128;
            const keep = self.dim - drop;
