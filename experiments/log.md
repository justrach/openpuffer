...
| E027 | 2026-08-25 | AVX-512 VNNI `vpdpbusd` for `dotI8` (one zmm acc; signed via XOR-0x80 + 128*sum(b) correction). Not AMX — tile setup loses on a single 1536-d vector. Keep E013 64-lane fallback. | llvm.x86.avx512.vpdpbusd.512 in dotI8; Debug stays on E013 widen (self-hosted `moveStrategy` TODO) | src/vector.zig | pass | 0.635 / 0.827 | 0.3105 | +36.6% / +77.8% vs fresh control 0.465 | discard | engine | control pair on current main: 0.493 / 0.465 ms, recall 0.3105. Historical E022 0.575 is stale-slow on this flatten tree (re-baseline). Both variant runs slower than control and than 0.575; recall held 0.3105. XOR+i16-reduce correction + still-memory-bound dots; E019-style extra work lost. Reverted vector.zig. |
