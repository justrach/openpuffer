...
| E027 | 2026-08-25 | drop the hot-path f32 copy (optional store_f32 / `--no-f32`) so 2M×1536 can fit on 16 GiB after E025 flatten | pending: Options.store_f32 default true; --no-f32 skips f32 slab; int8 traversal; rerank only if f32 present | src/hnsw.zig src/main.zig | pending | — | — | — | running | memory | MEMORY track. Scored bench command unchanged (default still stores f32). |
