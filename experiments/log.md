...
| E032 | 2026-08-26 | per-layer `visited.setRangeValue(false)` wipes ~20k bits on every layer; a visit-generation stamp avoids the wipe (hnswlib-style) | VisitStamp {stamps, epoch}; epoch++ per searchLayer; no bitset clear | src/hnsw.zig | pass | 0.526 / 0.510 | 0.3105 | +9.1% / +5.8% vs fresh control 0.482 | discard | engine | u32 stamps are a random 4B load per neighbor vs a dense bitset; slower. Control 0.482/0.499 recall 0.3105. Reverted. |
| E033 | 2026-08-26 | widen f32 `dot` 16→64 so rerank fills an AVX-512 register (E013 did this for i8; E021 was dual-16 not 64-wide) | lane=16→64 in vecmath.dot; math unchanged | src/vector.zig | pass | 0.524 / 0.524 | 0.3105 | +8.7% / +8.7% vs fresh control 0.482 | discard | engine | 64-wide f32 slower than 16-wide (exact scan also 5.56 vs ~5.0). Reverted vector.zig. Also finished the E032 revert: VisitStamp was still in hnsw.zig after the discard-ledger commit; restored DynamicBitSet from main. |
| E034 | 2026-08-26 | qvec pool (~30 MiB) is larger than LLC and each neighbor is touched once, so temporal prefetch (locality=3) may evict the query/visited working set; NTA (locality=0) should hide the load without polluting L2 | existing one-ahead `@prefetch` locality 3→0; not E018's extra lines | src/hnsw.zig | pass | 0.521 / 0.511 | 0.3105 | +8.1% / +6.0% vs fresh control 0.482 | discard | engine | NTA hint slower than temporal one-ahead. loop_once vs E022 0.575 is a false keep. Reverted. |
| E035 | 2026-08-26 | traversal is random 1536-byte i8 loads; scoring only the first 768 dims (12 AVX-512 lines) halves qvec traffic, then full f32 rerank restores order | QDist.search prefix = dim/2 (64-aligned) when dim≥512; insert/distPair stay full-width | src/hnsw.zig | pass | 0.320 | 0.1645 | −33.6% vs fresh control 0.482 | discard | engine | Fast but recall 0.1645 < 0.30 floor. Rerank cannot recover neighbors the prefix walk never visited. Reverted. |
| E036 | 2026-08-26 | E035's prefix walk was a real −33% but overshot quality; keep 1280/1536 dims (20 AVX-512 chunks, drop last 4 lines) so the walk stays closer to full cosine | QDist.search prefix = 5/6 dim, 64-aligned, dim≥512; insert stays full-width | src/hnsw.zig | pass | 0.389 | 0.2895 | −19.3% vs fresh control 0.482 | discard | engine | Still under the 0.30 floor (0.2895). Latency is real. Next: 1408-d if we stay on this axis. Reverted. |
| E037 | 2026-08-26 | interpolate E036 (1280d / 0.2895) and full (1536d / 0.3105): drop only the last 128 dims (2 AVX-512 chunks) so recall should sit on the 0.30 line | QDist.search prefix = dim-128, 64-aligned, dim≥512; insert stays full-width | src/hnsw.zig | pass | 0.476 / 0.462 | 0.3010 | −1.2% / −4.15% vs fresh control 0.482 | keep | engine | Run 2 decides (≥4%). Recall 0.3010 holds the floor (was 0.3105). Exact scan 4.90 ms (not an ambient dip). New host best on this flatten+RCU tree: 0.462 ms. Insert/distPair stay full-width. |
| E038 | 2026-08-26 | E026 showed extra f32 rerank slots do not recover neighbors the walk never saw; drop `rerank_mult * k` down to `k` so we skip 30 unused 1536-d f32 dots on the scored path | searchAdvanced rerank_n = min(res, k); `--rerank-mult` no longer expands the f32 set | src/hnsw.zig | fail | — | — | — | discard | engine | Gate fail: clustered dim=1536 test recall 0.800 (8/10). Extra rerank slots *are* needed on clustered data. No scored bench. Reverted. |
| E039 | 2026-08-26 | after popping a candidate, prefetch the next heap item's packed neighbor row so snapshotNeighbors memcpy hits L1 (graph CSR, not qvec; not E018) | @prefetch neighborSlotsConst(peek.id) at the start of each expansion | src/hnsw.zig | pass | 0.433 | 0.3010 | −6.3% vs E037 0.462 | keep | engine | Run 1 clears 4%. Recall bit-stable 0.3010. Exact scan 4.73 vs E037 4.90 (small ambient). New host best 0.433 ms. |
| E040 | 2026-08-26 | once the ef-set is full, remaining i8 dims cannot beat `w` if partial + 127·Σ|q| bound is below the cosine threshold — abandon those dots (conservative, same admissions) | QDist.distCutoff + suffix bound every 256 dims; only when results≥ef; insert stays full dist | src/hnsw.zig src/vector.zig | pass | 0.691 / 0.671 | 0.3010 | +59.6% / +55.0% vs E039 0.433 | discard | engine | Recall bit-stable (bound is conservative) but the extra reduce/check every 256 dims is far more expensive than skipped tail work. Exact 5.41 / 5.07. Reverted. |
| E041 | 2026-08-26 | most evals are layer 0; drop 192 i8 dims there (1344) while upper-layer greedy descent stays at the 1408-d walk so the entry point stays in the right neighborhood | searchPrefix(layer): L0 dim-192, upper dim-128, 64-aligned; refresh ep.d on L0; insert full-width | src/hnsw.zig | pass | 0.416 | 0.2815 | −3.9% vs E039 0.433 | discard | engine | Fast (at the 4% line) but recall 0.2815 < 0.30. No 64-aligned point between 1344 and 1408. Exact 4.73. Reverted. |
| E042 | 2026-08-26 | E041 lost recall because 1408-d upper + 1344-d L0 disagree; full-width upper descent should land a better L0 entry so 1344-d L0 can hold ≥0.30 | searchPrefix: layer>0 → full dim, layer 0 → dim-192; refresh ep.d; insert full-width | src/hnsw.zig | pass | 0.421 | 0.2845 | −2.8% vs E039 0.433 | discard | engine | Recall 0.2845 still under 0.30 (only +0.003 vs E041). Full-width upper does not recover the L0 prefix quality hole. Exact 5.08. Reverted. Prefix-shorter than 1408 is spent. |
