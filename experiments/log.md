...
| E040 | 2026-08-26 | once the ef-set is full, remaining i8 dims cannot beat `w` if partial + 127·Σ|q| bound is below the cosine threshold — abandon those dots (conservative, same admissions) | QDist.distCutoff + suffix bound every 256 dims; only when results≥ef; insert stays full dist | src/hnsw.zig src/vector.zig | pending | — | — | — | running | engine | |
